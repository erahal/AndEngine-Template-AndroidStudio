...
public class BatchedPseudoSpriteParticleSystem extends BlendFunctionParticleSystem<Entity> {
	...
	public BatchedPseudoSpriteParticleSystem(final IParticleEmitter pParticleEmitter, final float pRateMinimum,
			final float pRateMaximum, final int pParticlesMaximum, final ITextureRegion pTextureRegion,
	...
	public BatchedPseudoSpriteParticleSystem(final float pX, final float pY, final IParticleEmitter pParticleEmitter,
	...
	protected void onManagedDraw(final GLState pGLState, final Camera pCamera) {
		...
		for (int i = this.mParticlesAlive - 1; i >= 0; i--) {
			...
			/*
			 * In order to support alpha changes of the sprites inside the
			 * spritebatch, we have to 'premultiply' the RGB channels of the
			 * sprite with its alpha channel.
			 */
			final float alpha = entity.getAlpha();
			final float colorABGRPackedInt = ColorUtils.convertRGBAToABGRPackedFloat(entity.getRed() * alpha, entity.getGreen()
					* alpha, entity.getBlue() * alpha, alpha);

			this.mSpriteBatch.drawWithoutChecks(this.mTextureRegion, entity, this.mTextureRegion.getWidth(),
					this.mTextureRegion.getHeight(), colorABGRPackedInt);
		}
		this.mSpriteBatch.submit();

		this.mSpriteBatch.on_Draw(pGLState, pCamera);
