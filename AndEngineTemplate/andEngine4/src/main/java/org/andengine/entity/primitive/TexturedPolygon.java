...
public class TexturedPolygon extends TexturedMesh {
	 ...
	 */
	public TexturedPolygon(final float pX, final float pY, final float[] pVertexX, final float[] pVertexY,
	 ...
	 * {@link Mesh#VERTEXBUFFEROBJECTATTRIBUTES_DEFAULT}.
	 */
	public TexturedPolygon(final float pX, final float pY, final float[] pVertexX, float[] pVertexY,
			final ITextureRegion pTextureRegion, final VertexBufferObjectManager pVertexBufferObjectManager,
			final DrawType pDrawType) {
		this(pX, pY, buildVertexList(mTriangulator.computeTriangles(buildListOfVector2(pVertexX, pVertexY))),
				VERTEX_SIZE_DEFAULT_RATIO, pTextureRegion, pVertexBufferObjectManager, pDrawType);
	 ...
	 * {@link Mesh#VERTEXBUFFEROBJECTATTRIBUTES_DEFAULT}.
	 */
	public TexturedPolygon(final float pX, final float pY, final float[] pBufferData, final float sizeRatio,
			final ITextureRegion pTextureRegion, final VertexBufferObjectManager pVertexBufferObjectManager,
			final DrawType pDrawType) {
		super(pX, pY, (int) ((pBufferData.length / VERTEX_SIZE) * sizeRatio), DrawMode.TRIANGLES, pTextureRegion,
				new HighPerformanceTexturedMeshVertexBufferObject(pVertexBufferObjectManager, pBufferData,
						(int) ((pBufferData.length / VERTEX_SIZE) * sizeRatio), pDrawType, true,
	 ...
	 * @param pVertexX
	 * @param pVertexY
	 * @return true if vertices were correctly updated false otherwise
	 */
